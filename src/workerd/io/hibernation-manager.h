...
public:
  ...
  friend jsg::Value api::HibernatableWebSocketEvent::getError(jsg::Lock& lock);
...
private:
  ...
  public:
    ...
    void unhibernate(jsg::Lock& js) {
      auto a = manager.deserializeV8Value(attachment, js.v8Isolate);
      activeWebSocket.emplace(api::WebSocket::unhibernate(js, *ws, a, url, protocol, extensions));
    }
...
private:
  ...
  v8::Local<v8::Value> deserializeV8Value(kj::ArrayPtr<const kj::byte> buf, v8::Isolate* isolate);
  ...
  struct PackageForHandler {
    kj::Maybe<HibernatableWebSocket&> currentOccupant;
    kj::Maybe<kj::Exception> e;
  };
  PackageForHandler package;
  // Allows the HibernatableWebSocket event handler that is currently running to access the
  // HibernatableWebSocket and other properties that it needs to execute.
