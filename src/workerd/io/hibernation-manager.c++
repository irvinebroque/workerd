...
v8::Local<v8::Value> HibernationManagerImpl::deserializeV8Value(
  ...
  jsg::Deserializer deserializer(isolate, buf, nullptr, nullptr, jsg::Deserializer::Options {
    .version = 15,
    .readHeader = true,
  });
...
kj::Promise<void> HibernationManagerImpl::readLoop(HibernatableWebSocket& hib) {
  ...
  return ws.receive()
    ...
    // Create the package that will be opened by the event handler.
    // TODO(now): We still need to figure out when we set the error.
